...
| Local core | 66 tests pass, including 22 exporter ACK/outage/restart/quarantine tests, native action JSON normalization and strict audit checks; 31 runtime tests passed in the uploaded release bundle. | R2 exporter hardening is local source, not Edge deployment. Runtime JSON fix deployed via pinned descriptor; later strict audit remains local. The older [37-test log](results/merged_core_tests.log) is historical. |
...
| VPS development preview | `103.27.132.88`: service enabled, replay homepage/assets/API/SSE verified; nginx Docker upstream `http://172.23.0.1:5173`. | Deployed frontend `0052c5272e5e4cc1215741427445bd42be5af8c8`; [verification](deploy/vps-preview/verification.json). Persistent state, repeat update and failure rollback tested. Replay only; AI/commands disabled; public nginx/TLS remains user-configured. |
...
Routes below use `/api/v1`. The VPS replay preview is reachable from its local nginx Docker container; public nginx/TLS and actual Edge-container transport acceptance remain unverified. **No user authentication exists** on snapshot/SSE/incidents/chat/command creation; same-origin checks are not authentication. Public deployment requires user/team/run authorization and durable rate/cost controls.
