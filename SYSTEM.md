...
| VPS development preview | https://hackathon.aquatictw.com/ loads persisted replay evidence and connects SSE in an actual browser. `103.27.132.88`, nginx Docker upstream `http://172.23.0.1:5173`. | Deployed frontend `0052c5272e5e4cc1215741427445bd42be5af8c8`; [verification](deploy/vps-preview/verification.json). Persistent state, repeat update and failure rollback tested. Replay only; AI/commands disabled. Public HTTPS is now observed; it is not live Edge transport. |
| Integrated backend/frontend | R2 command creation/ACK writes enforce atomic scope/state/expiry guards; dashboard lifecycle isolates stale requests and retains same-scope reconnect data. 81 frontend tests, TypeScript and production build pass. | [R2 integration checks](results/integration_r2_20260919.json); earlier [R1 checks](results/integration_20260919.json) remain historical. No live machine transport acceptance. |
...
Validation: `python -B -m unittest discover -s grp6_app/tests -v` passes 66 tests on September 19, including 22 exporter tests with real localhost lost-ACK/duplicate and outage/restart cases. These are local tests, not actual-container transport acceptance. Queue capacity is bounded by event count, not bytes; deep-copy callback cost remains unmeasured on Edge. SQLite/quarantine disk use is unbounded and requires operational monitoring/manual review; no repair/requeue CLI is supplied. Shutdown attempts a queue flush with bounded caller wait; process loss before COMMIT or unavailable storage can still lose volatile submissions. Completeness/live SDK semantics and actual-container TLS remain open.
...
Isolated Node 22.18 leaves global Node unchanged. The VPS CPU requires workerd-only `qemu-x86_64 -cpu max` emulation, so startup/requests are slower. Fourteen labeled replay events are seeded idempotently; paid AI and command tokens are absent. This preview demonstrates development UI/API behavior, not live Edge transport, browser interaction acceptance or tester receipts. Source `0052c52` is deployed; later A exporter/docs/deployment-helper commits do not change that frontend tree.
