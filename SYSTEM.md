...
| Integrated backend/frontend | D per-site projection/measurement API and E scoped UI/command states merged. 52 combined tests, TypeScript and production build pass; A fixed mixed-source/ambiguous joins and command receipt projection. | [Integration checks](results/integration_20260919.json). No new hosted or machine transport acceptance. Older frontend logs remain historical. |
