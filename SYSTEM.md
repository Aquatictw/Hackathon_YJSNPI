...
1. Remote `main` is the sole publication branch. Each role works in its own checkout/local branch: `teammate-a-core`, `teammate-b-prediction`, `teammate-c-detection`, `teammate-d-backend`, `teammate-e-frontend`. A publishes a numbered round and exact handoff SHA; new work starts there, never at a permanently hardcoded old base. Preserve unfinished work and never switch another agent's checkout. Existing assignments describe the completed first round; await A's next numbered handoff before repeating them.
