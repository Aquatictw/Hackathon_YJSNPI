...
| Production run 3 | 80 devices, 74 pass / 6 fail; 120 requests, 480/480 actual joins; continuous sequence 1–765. | [Capture audit](results/vm_production/local_audit.json), [report](results/vm_production/grp6_core_prod3_report.html); max recorded callback latency 8.350 ms. |
| Prediction receipts | 120/120 exact action matches with successful adjacent execution. Three alerts at device 32 returned by get_prod. | [Receipt audit](results/vm_production/tester_receipt_audit.json): anomaly tester receipt still unverified. |
| Local core | 41 tests pass, including native action JSON normalization; 31 runtime tests pass in the extracted bundle. | September 19 local checks; the older [37-test log](results/merged_core_tests.log) is historical. Not deployed. |
...
4. If needed, in /home/user/Case_Event/SmarTest run `DISPLAY=:1 XAUTHORITY=/home/user/.Xauthority bash runTp.sh load`; wait for SMT8 Ready. Start `/opt/acs/nexus/bin/AppDeployer start`, verify communication/events, then `bash runTp.sh eng_run 1`.
5. Production uses `bash runTp.sh prod_run`; launcher rebuilds SmarTest/TCCT and may remove deployment. Verify app readiness before lot start and check stale AppDeployer state. Second loop argument is not a one-run guarantee; bound TCCT recipe execution avoided teardown in recorded run 3.
...
B writes only new workstreams/prediction/ files; C only new workstreams/detection/ files. Both deliver offline proposals; A reviews/promotes accepted runtime/artifact changes.
...
1. A publishes the documentation cleanup commit. Each teammate branches from that exact commit in a separate checkout/worktree: team/b-prediction, team/c-detection, team/d-backend, team/e-frontend. Do not substitute pre-cleanup HEAD.
