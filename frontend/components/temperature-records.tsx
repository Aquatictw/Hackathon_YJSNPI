      ...
      <table className="dc-temperature-table"><thead><tr><th scope="col">{t("Stage / Site")}</th><th scope="col">{t("Predicted")}</th><th scope="col">{t("Actual")}</th><th scope="col">{t("Difference")}</th><th scope="col">{t("Record details")}</th></tr></thead>
        <tbody>{predictions.map(row => {
          ...
          return <tr key={row.event_id}>
            ...
            <td className="dc-temperature-estimate"><strong className="dc-temperature-value" title={t("Exact prediction: {0}", row.prediction ?? t("Not provided"))}>{row.prediction == null ? '—' : displayNumber(row.prediction)}</strong>{row.unit && <small>{row.unit}</small>}</td>
            <td className="dc-temperature-actual"><strong className="dc-temperature-value" title={t("Exact actual: {0}", row.actual ?? t("Not provided"))}>{row.actual == null ? '—' : displayNumber(row.actual)}</strong>{(row.actual_status || row.actual == null || row.unit) && <small>{row.actual_status ? t(row.actual_status === '實測衝突' ? 'Conflicting actuals' : 'Ambiguous actual scope') : row.actual == null ? t("Awaiting measurement") : row.unit}</small>}</td>
