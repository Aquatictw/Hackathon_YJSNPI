...
import { createRunDiscovery, initialRunDiscovery, isRecordedCapture, isTrainingReplay, runChoiceKey, storedRunName, storedRunSourceLabel, selectedStoredRun } from '@/lib/rtdi/run-discovery';
...
export function StoredRunPicker({ onLoad, busy = false, imported = false, children, excludeTraining = false, onLoadSummary, summarySelected = false }: Props) {
  ...
  const canLoad = !busy && (manual ? Boolean(run.trim()) : (archive && Boolean(onLoadSummary)) || (Boolean(selected) && !state.loading));
  ...
  return <section className="dc-run-picker" aria-label={t('Run selection')}>
    <form id={imported ? 'load-backend-run' : undefined} className="dc-connect" aria-busy={busy} onSubmit={event => {
      ...
      {manual ? <>
        ...
        <select id={id + '-stored'} value={archive ? '__summary__' : state.selected} aria-describedby={id + '-status ' + id + '-note'} disabled={!onLoadSummary && (state.loading || !state.runs.length)} onChange={event => {setArchive(event.target.value === '__summary__'); discovery.current?.select(event.target.value);}}>
          ...
          {(['replay', 'live'] as const).map(category => <optgroup key={category} label={t(category === 'live' ? 'Live' : 'Replay')}>
            ...
            {state.runs.filter(item => category === 'live' ? item.mode === 'live' : item.mode !== 'live').map(item => <option key={runChoiceKey(item)} value={runChoiceKey(item)}>{storedRunName(item) ? `${t(storedRunName(item)!)} · ` : ''}{t(storedRunSourceLabel(item))} · {item.tester_id} / {item.run_id}</option>)}
          </optgroup>)}
        </select>
      </label>}
