...
function EvidencePlot({alert}:{alert:ReplayAlert}){
 ...
 return <div className="replay-plot"><div className="plot-caption"><strong>{yieldMode ? t("Cumulative yield") : t("Measurement sequences")}</strong>{yieldMode && <span>{t("Percent")}</span>}</div><PlotAxes perSite={!yieldMode && !!Object.values(alert.site_series ?? {}).some(values => values.length)} metric={yieldMode ? 'yield' : 'measurement'}><svg viewBox="0 0 650 260" role="img" aria-label={t("{0}; {1}; x-axis is sample order, not time.", t(alertNames[alert.kind]), lines.map(l=>l.name).join(', '))}>
 ...
 </svg></PlotAxes><div className="plot-legend">{lines.map((l,i)=><span key={l.name}><i style={{background:colors[i%colors.length]}}/>{t(l.name)}</span>)}<span className="axis-note">{yieldMode ? t("Completed-device order") : t("Sample order within each site")} {t("· no timestamps")}</span></div></div>;
}
