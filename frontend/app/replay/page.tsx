...
import {RunAnalysis,RunAnalysisSourceChooser} from '@/components/run-analysis';
import {ReplayArchiveNotice} from '@/components/connection-status';
import {initialAnalysisSource,selectAnalysisSource,type AnalysisSource} from '@/lib/rtdi/run-analysis';
...
export default function ReplayPage(){
 const {t}=useLocale();
 // Keep server HTML and the hydration pass identical; browser storage and
 // source lifecycles are accessed only by the mounted client subtree.
...
function ReplayClient(){
 const {t}=useLocale();
 const [source,setSource]=useState<AnalysisSource>(initialAnalysisSource),[sourceError,setSourceError]=useState('');
 const chooser=<><RunAnalysisSourceChooser source={source} onChange={next=>{
  if(next===source)return;
  try{selectAnalysisSource(next);setSource(next);setSourceError('');}
  catch{setSourceError('The source choice could not be saved. Allow browser session storage and retry.');}
 }}/>{sourceError&&<p className="dc-error" role="alert">{t(sourceError)}</p>}</>;
 return source==='backend'?<RunAnalysis sourceChooser={chooser}/>:<OfflineReplayAnalysis sourceChooser={chooser}/>;
}
function OfflineReplayAnalysis({sourceChooser}:{sourceChooser:ReactNode}){
 const {t} = useLocale();
