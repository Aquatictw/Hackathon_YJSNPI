...
function BackendWorkspace({ initialScope, onLoadSummary, summaryLoading }: { initialScope?: BackendScope; onLoadSummary: () => Promise<void>; summaryLoading: boolean }) {
 ...
 {data && <dl className="dc-loaded-run">
  <div><dt>{t('Loaded run')}</dt><dd>{data.run.run_id}</dd></div>
  <div><dt>{t('Tester ID')}</dt><dd>{data.run.tester_id}</dd></div>
  <div><dt>{t('Last source event')}</dt><dd>{lastSourceEvent && !Number.isNaN(lastSourceEvent.getTime()) ? <time dateTime={lastSourceEvent.toISOString()}>{lastSourceEvent.toLocaleString(locale, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', timeZoneName: 'short' })}</time> : data.run.last_event_at}<SourceFreshness source={data.run}/></dd></div>
 </dl>}
 ...
 <footer className="dc-footer"><span><Database size={13}/>{data ? `${data.run.run_id} / ${data.run.tester_id}` : t("No backend snapshot")}{data && t(" · Last source event: {0} · Stored data does not establish current tester connectivity", data.run.last_event_at)}</span><span>RTDI / GRP6</span></footer></main></div></div>;
}
