...
export default function Dashboard() {
    ...
    const ask = (q: string) => { if (ai.enabled)
        return lifecycle.current?.ask(q); };
 ...
 </div></section><aside className="dc-ai" aria-labelledby="investigation-heading"><div className="dc-ai-heading"><NotebookPen size={18}/><div><h2 id="investigation-heading">Investigation log</h2><small>Evidence-linked analysis</small></div><span className="dc-readonly">Read only</span></div><div className="dc-ai-context"><span>Context</span><strong>{current ? `W${current.wafer_id ?? '—'} · Site ${current.site_id ?? 'All'} · ${names[current.kind ?? ''] ?? current.kind}` : scope?.run ?? 'No run selected'}</strong></div>{!ai.enabled && <div className="dc-service-state" role="status"><strong>{ai.title}</strong><p>{ai.message}</p><div><button type="button" className="dc-secondary" onClick={() => setConfigAttempt(n => n + 1)}>Retry service check</button><a href="/sandbox">Open sandbox</a></div></div>}<div className="dc-chat" aria-live="polite">{!messages.length ? <div className="dc-ai-welcome"><span className="dc-notebook-label">NEW INVESTIGATION</span><h3>No entries for this context</h3><p>Select an evidence record and draft a question. Responses reference records from the loaded run.</p><div className="dc-draft-label">Question templates <span>Draft only</span></div>{['Summarize the evidence for this anomaly.', 'Compare the affected sites and identify differences.', 'What additional measurements would test this finding?'].map(q => <button key={q} disabled={!scope || busy || !ai.enabled} onClick={() => { lifecycle.current?.setQuestion(q); document.getElementById('investigation-question')?.focus(); }}>{q}<ArrowRight size={14}/></button>)}</div> : messages.map((m, i) => <div key={i} className={`dc-chat-message ${m.role}`}><small>{m.role === 'user' ? 'QUESTION' : 'ANALYSIS'}</small><Answer text={m.text}/>{m.refs?.length ? <div className="dc-citations">{m.refs.map(id => <button key={id} disabled={!evidence.some(record => record.evidence_id === id)} title={evidence.some(record => record.evidence_id === id) ? "Open cited evidence" : "Cited evidence is not in the current snapshot"} onClick={() => { const e = evidence.find(e => e.evidence_id === id); if (e) {
        lifecycle.current?.selectEvidence(e.event_id, true);
        setTab('evidence');
    } }}>↗ {id}</button>)}</div> : null}{m.id && <small>Investigation ID {m.id}</small>}</div>)}{busy && <div className="dc-thinking"><RefreshCw size={15}/>Retrieving evidence and preparing response…</div>}</div>{chatError && <div className="dc-error" role="alert">{chatError}</div>}<form className="dc-composer" onSubmit={e => { e.preventDefault(); void ask(question); }}><textarea id="investigation-question" aria-label="Investigation question" aria-describedby="ai-service-note" placeholder={!ai.enabled ? ai.title : scope ? 'Ask a question about this run…' : 'Load a run to start an investigation…'} value={question} onChange={e => lifecycle.current?.setQuestion(e.target.value)} maxLength={2000} disabled={!scope || busy || !ai.enabled}/><div><span>One request per submission</span><button className="dc-primary" aria-label="Submit question" disabled={!scope || busy || !ai.enabled || !question.trim()}><Send size={15}/>Submit</button></div></form><p id="ai-service-note" className="dc-ai-foot">Submitting uses the model API. Completed answers and citations are stored by the service. Generated analysis is advisory and does not confirm tester receipt.</p></aside></div>
