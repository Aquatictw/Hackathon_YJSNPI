...
export default function Home(){
 ...
 function reset(){copySeq.current++;setCopied(false);cancelInvestigation();const fresh=emptyWorkspace();stateRef.current=fresh;setWorkspace(fresh);setSelected(null);setChats({});setAiError("");setInputError("");setQuestion("");setRaw("");setPracticeEvents({});setPracticeScenario("healthy");setScenario("anomaly");setAiMode("demo");setNotice("Local workspace cleared. Tester data is unchanged.");}
 ...
 <h2>{current.incident?.title ?? t(kindLabels[current.event.kind])}</h2><p className="event-message">{current.event.message}</p>
