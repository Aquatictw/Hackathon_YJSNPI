...
function Trend({evidence:e}: {evidence:EvidenceRecord}){
 ...
 const lines=e.metric!=='yield'&&sites.length?sites.map(([site,values])=>({label:`Site ${site}`,points:values.map((value,index)=>({index:index+1,value}))})):[{label:'觀測值',points:e.series}];
 const points=lines.flatMap(line=>line.points);
 if(!points.length)return <div className="no-chart"><CircleDashed size={30}/><strong>未提供趨勢序列</strong><span>不以缺值補零，也不將缺資料視為正常。</span></div>;
 ...
 return <div className="trend"><div className="chart-title"><span>測試結果趨勢 <small>{e.metric==='yield'?'良率 %':e.unit}</small></span><div className="legend">{lines.map((line,i)=><span key={line.label}><i style={{borderColor:colors[i%colors.length]}}/>{line.label}</span>)}</div></div><svg viewBox="0 0 650 235" role="img" aria-label="來源序列，橫軸為已完成裝置順序，不是時間">
 ...
 {[0,Math.floor((count-1)/2),count-1].map((i,k)=><text key={k} x={x(i)} y="214" textAnchor="middle">{lines[0].points[i]?.index??i+1}</text>)}<text x="620" y="232" textAnchor="end">{sites.length&&e.metric!=='yield'?'各 site 已完成裝置順序':'已完成裝置順序'}</text>
 </svg></div>;
}
export default function Home(){
 ...
 return <div className="shell"><header className="topbar"><a className="brand" href="/"><span className="brandmark"><Activity size={22}/></span> RTDI <span className="brandlight">Insight</span></a><div className="header-right"><ThemeSelector/><a className="mode" href="/replay"><Activity size={14}/> 重播分析</a><span className="mode"><FlaskConical size={14}/> 本機原型</span><span className="team">G6</span></div></header>
 ...
 {e?<><div className="evidence-metrics"><div><span>{e.metric==='yield'?'良率':e.metric==='coverage'?'特徵完整度':e.metric==='measurement'?'觀測值':'窗口均值'}</span><strong>{e.observed===null?'—':(e.metric==='coverage'||e.metric==='yield')?`${(e.observed*100).toFixed(1)}%`:e.observed.toFixed(4)}<small>{(e.metric==='coverage'||e.metric==='yield')?'':e.unit}</small></strong></div><div><span>參考基準</span><strong>{e.baseline===null?'—':e.metric==='yield'?`${(e.baseline*100).toFixed(1)}%`:e.baseline}</strong></div><div><span>樣本數</span><strong>{e.sample_count}<small>筆</small></strong></div></div><Trend evidence={e}/><div className="evidence-foot"><span>{e.test_name}</span><span>Evidence · {e.evidence_id}</span><span>來源門檻：{e.threshold??'未提供'} · 尺度未確認</span>{e.response_status&&<span>回覆：{responseLabels[e.response_status]} · 回執：{e.tester_receipt_id??'未提供'}</span>}</div>{e.missing_fields.length>0&&<div className="missing-fields"><Info size={15}/><span>缺少：{e.missing_fields.join(', ')}</span></div>}</>:<div className="no-chart"><Info/><strong>尚未收到對應 evidence</strong><span>請等待後續批次，不能僅依 message 確認根因。</span></div>}
 <div className="delivery"><div><Bell size={15}/><strong>機台回傳</strong><span>尚未建立命令</span></div><div className="delivery-steps" style={{flexWrap:"wrap"}}><span>訊息草稿</span><ChevronRight/><span>後端排隊</span><ChevronRight/><span>Edge 接收</span><ChevronRight/><span>機台訊息排隊</span><ChevronRight/><span>機台回執</span></div><p>此原型不發送機台命令。AI 回答不會改變回傳狀態。</p></div>
 </div>}</TabsContent>
 <TabsContent value="predictions"><div className="prediction-body"><div className="section-title"><h3>六階段溫度預測</h3><span>與所選事件綁定</span></div><p className="sub">示範值不代表實際模型精度。回覆排隊與機台確認是不同狀態。</p>{current?.predictions.length?<div className="table-scroll"><table><thead><tr><th>階段</th><th>預測／實測</th><th>完整度</th><th>回覆狀態</th></tr></thead><tbody>{current.predictions.map(p=><tr key={p.prediction_id}><td><span className="stage-number">0{p.stage}</span><details><summary>預測來源</summary><small>每 site 請求：{p.request_id??'未提供'}</small><small>原始請求：{p.original_request_id??'未提供'}</small><small>來源事件：{p.source_event_id??'未提供'}</small></details></td><td><strong>{p.predicted?.toFixed(2)??'—'}</strong><span className="actual"> / {p.actual?.toFixed(2)??'—'}</span><small>{p.unit}</small></td><td>{p.coverage===null?'未知':`${Math.round(p.coverage*100)}%`}</td><td><span className={`prediction-status ${p.response_status==='insufficient_data'?'orange':''}`}>{responseLabels[p.response_status]}</span>{p.tester_receipt_id&&<small>{p.tester_receipt_id}</small>}</td></tr>)}</tbody></table></div>:<div className="no-chart"><Cpu/><strong>尚無預測紀錄</strong><span>接收一組 message 後，即可查看對應範例。</span></div>}</div></TabsContent>
